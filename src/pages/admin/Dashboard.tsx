...
const monthly = [
  { m: "Jan", v: 60 }, { m: "Feb", v: 90 }, { m: "Mar", v: 70 },
  { m: "Apr", v: 130 }, { m: "May", v: 160 }, { m: "Jun", v: 200 }, { m: "Jul", v: 50 },
];
...
export default function AdminDashboard() {
  ...
  const total = events.length || 1248;
  const completed = events.filter((e) => e.status === "completed").length || 982;
  const inProgress = events.filter((e) => e.status === "active").length || 145;
  ...
  const totalV = vehicles.length || 430;
  const availPct = Math.round((ownCount / Math.max(totalV, 1)) * 100) || 85;

  const donut = [
    { name: "Driver/Owner", value: ownCount || 240, fill: "#2563eb" },
    { name: "Driver", value: rentCount || 45, fill: "#dbe1ff" },
    { name: "Agency", value: agencyCount || 12, fill: "#943700" },
  ];
    ...
    <AdminLayout>
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <Kpi icon={Calendar} iconBg="bg-blue-100 text-blue-600" label="Total Events" value={total.toLocaleString()} delta="12%" deltaUp accent="bg-blue-500" />
          <Kpi icon={CheckCircle2} iconBg="bg-emerald-100 text-emerald-600" label="Completed" value={completed.toLocaleString()} delta="8%" deltaUp accent="bg-emerald-500" />
          <Kpi icon={Clock} iconBg="bg-amber-100 text-amber-600" label="In Progress" value={inProgress.toLocaleString()} delta="4%" deltaUp accent="bg-amber-500" />
          <Kpi icon={Truck} iconBg="bg-red-100 text-red-600" label="Total Vehicles" value={totalV.toLocaleString()} delta="2%" deltaUp={false} accent="bg-red-500" />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Monthly Events Overview</h3>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Driver Availability</h3>
            <div className="relative h-56 mt-2">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div className="text-2xl font-bold text-slate-900">{availPct}%</div>
